...
def scrape_comprehensive(session, out='data/pls_comprehensive.json'):
    os.makedirs('data', exist_ok=True)
    results = []
    for section in ['cases', 'statutes', 'rules', 'orders']:
    ...
    print(f"[+] Saved {len(results)} comprehensive records to {out}")

if __name__ == '__main__':
    s = requests.Session()
    if login(s):
        scrape_comprehensive(s)
    else:
        print("[-] Login failed")
        sys.exit(1)
